...
Zeabur can deploy this project directly from the Dockerfile. The bot uses `PORT` to bind the web server, so the only required setup is environment variables.

## Steps

1. **Create a new project** in Zeabur and choose **Deploy from GitHub** (or upload the repo).
2. **Select Docker build** when prompted (Zeabur will use the included `Dockerfile`).
3. **Set required environment variables** in Zeabur:
   - `BOT_TOKEN`
   - `OWNER_ID`
   - `TELEGRAM_API`
   - `TELEGRAM_HASH`
4. **Optional but recommended:**
   ...
   - `BASE_URL` (set to your Zeabur domain, e.g. `https://your-app.zeabur.app`)
   - `BASE_URL_PORT` (leave `80` unless you need a custom port)
   - `RCLONE_SERVE_PORT` (default `8080` if you expose rclone serve)
5. **Deploy** and wait for the build to finish.

> Tip: You can copy the keys from `.env.example` into Zeabur’s environment panel.
